...
def route_after_cicd(state: EvaluatorState) -> Literal["reviewer", "error_handler", "skip", "cicd", "orchestrator"]:
    ...
    cicd_retry_result = state.get("cicd_retry_result", {})

    if cicd_retry_result.get("requested"):
        if cicd_retry_result.get("exhausted"):
            return "error_handler"
        if cicd_retry_result.get("retry_mode") in {"reassemble", "revalidate_only", "rerun_batch"}:
            return "orchestrator"
        return "cicd"
...
def prepare_cicd_retry(state: EvaluatorState) -> Dict[str, Any]:
    ...
    retry_mode = retry_result.get("retry_mode") or state.get("cicd_retry_mode")
...
def route_after_reviewer(state: EvaluatorState) -> Literal["report_fix_plan", "reporter", "orchestrator"]:
    """ReviewerAgent 后的路由
    
    Returns:
        report_fix_plan: 发现问题，进入修复决策
        reporter: 无问题，继续生成报告
        orchestrator: 正常完成，返回编排器
    """
    review_result = state.get("review_result", {})
