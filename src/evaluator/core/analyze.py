...
def _analyze_with_graph(
    ...
    # 非交互模式也需要 checkpointer 支持 interrupt/resume
    checkpointer = None
    try:
        ...
        pass
    graph = create_main_graph(llm_config=llm_config, storage_dir=str(version_dir), checkpointer=checkpointer)
    ...
    # 非交互模式：遇到 interrupt 时自动使用默认选项，不等待用户输入
    # 交互模式：保持原始 invoke 行为，由调用侧决定如何处理 interrupt
    from langgraph.types import Command
    config = {"configurable": {"thread_id": f"analyze-{int(time.time())}"}}
    ...
    if interactive_mode:
        ...
        while True:
            ...
            # 检测 interrupt：非交互模式下使用默认值自动 resume
            ...
            default_value = payload.get("default", "supplement") if isinstance(payload, dict) else "supplement"
            current_input = Command(resume=default_value)
