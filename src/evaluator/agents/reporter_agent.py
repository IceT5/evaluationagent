...
class ReporterAgent(BaseAgent):
    ...
    def run(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        section_assignment_result = state.get("section_assignment_result") or {}
        ...
        if section_assignment_result.get("workflow_assignments"):
            workflow_details = self._workflow_details_from_assignment(section_assignment_result)
        ...
        html_content = self._generate_html(
            ...
            organized_stage_details=state.get("organized_stage_details") or {},
        )
    ...
    def _workflow_details_from_assignment(self, section_assignment_result: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        details: Dict[str, Dict[str, Any]] = {}
        for assignment in section_assignment_result.get("workflow_assignments", []):
            name = assignment.get("workflow_id", "")
            if not name:
                continue
            numbering = assignment.get("numbering", "")
            detail_content = assignment.get("detail_markdown", "")
            desc = ""
            if detail_content:
                lines = [line.strip() for line in detail_content.splitlines() if line.strip()]
                for line in lines:
                    if line.startswith("**目的**") or line.startswith("- "):
                        desc = line.replace("**目的**", "").replace(":", "", 1).strip()
                        break
            ...
            details[anchor] = payload
            details[name] = payload
            details[name.replace('.yml', '')] = payload
            details[name.replace('.yaml', '')] = payload
        return details
    ...
    def _extract_scripts_section_from_md(self, content: str) -> str:
        """从 Markdown 内容中提取脚本目录索引章节（含关键配置详细分析）"""
        ...
        if config_match:
            return (
                f"## 脚本目录索引{scripts_body}"
                f"## 关键配置详细分析{config_match.group(1)}"
            )
        return f"## 脚本目录索引{scripts_body}"
    ...
    def _generate_html(
        ...
        statistics: dict,
        ci_data_path: str,
        organized_stage_details: dict | None = None,
    ) -> str:
        """生成交互式 HTML"""
        ...
        stage_contents = self._generate_stage_contents(
            architecture_data, workflow_details, ci_data_path, organized_stage_details or {}
        )
            ...
            document.querySelectorAll('.search-result-item').forEach(item => {{
                item.addEventListener('click', () => {{
                    ...
                    if (target) {{
                        ...
                        if (parentSection) {{
                            const contentId = parentSection.id + '-content';
                            const sectionContent = document.getElementById(contentId);
                            if (sectionContent) {{
                                if (parentSection.classList.contains('stage-section')) {{
                                    sectionContent.classList.add('expanded');
                                }} else {{
                                    sectionContent.classList.remove('collapsed');
                                }}
                                const btn = parentSection.querySelector('.toggle-btn');
                                if (btn) btn.textContent = '折叠';
                            }}
                        }}
                        // 展开 workflow card body（如果折叠）
...
'''
    ...
    def _generate_stage_contents(
        self,
        architecture_data: dict,
        workflow_details: dict,
        ci_data_path: str | None = None,
        organized_stage_details: dict | None = None,
    ) -> str:
        """根据架构图层生成各阶段内容。

        优先使用 organized_stage_details（真相源）渲染工作流卡片，
        architecture_data 仅用于确定阶段顺序和名称。
        """
        layers = architecture_data.get("layers", [])
        sections = []
        ...
        # 从 organized_stage_details 建立 stage_id -> workflows 映射
        stage_workflows_map: dict = {}
        if organized_stage_details and organized_stage_details.get("stages"):
            for stage in organized_stage_details["stages"]:
                stage_id = stage.get("stage_id", "")
                stage_workflows_map[stage_id] = stage.get("workflows", [])

        import re as _re
        for stage_idx, layer in enumerate(layers, start=1):
            layer_name = layer.get("name", f"阶段{stage_idx}")
            layer_id = layer.get("id") or f"stage-{stage_idx}"
            html_layer_id = f"stage-{stage_idx}"

            workflow_cards = []
            external_system_details = ""

            # 优先从 organized_stage_details 取该阶段的工作流列表
            stage_workflows = stage_workflows_map.get(layer_id, [])

            if stage_workflows:
                for wf_idx, wf in enumerate(stage_workflows, start=1):
                    wf_name = wf.get("workflow_id", "")
                    detail_markdown = wf.get("detail_markdown", "")
                    # 重新编号，统一为 stage_idx.wf_idx
                    if detail_markdown:
                        new_md, n = _re.subn(
                            r'^#{3,4}\s+\d+(?:\.\d+)*\.?\s+',
                            f'### {stage_idx}.{wf_idx} ',
                        ...
                        detail_markdown = new_md
                    card_html = f'''
            <div class="workflow-card" id="{self._to_anchor(wf_name)}">
                <div class="workflow-card-header">
                    <h4>{stage_idx}.{wf_idx} {wf_name}</h4>
                    <div>
                        <div class="meta">
                ...
                <div class="workflow-card-body collapsed">
                    {self._md_to_html(detail_markdown) if detail_markdown else '<p>暂无详情</p>'}
                </div>
            </div>'''
                    workflow_cards.append(card_html)
            else:
                # 回退：直接从 architecture_data nodes 渲染简单卡片（不做 workflow_details 匹配，避免子串误命中）
                nodes = layer.get("nodes", [])
                for node in nodes:
            ...
            section_html = f'''
    <section id="{html_layer_id}" class="section stage-section" data-title="{layer_name}">
        <div class="section-header">
            <h2 class="stage-title">{layer_name}</h2>
            <button class="toggle-btn" data-target="{html_layer_id}-content">展开</button>
        </div>
        <div id="{html_layer_id}-content" class="section-content">
    ...
    def _md_to_html(self, md_content: str) -> str:
        ...
        while i < len(lines):
            ...
            if re.match(r'^\s*[-*]\s+', line):
                ...
                if list_type == 'ol':
                    list_buffer.append(f'<li value="{num}">' + re.sub(r'^\s*\d+\.\s+', '', line) + '</li>')
                else:
                    flush_list()
                    list_type = 'ol'
                    list_buffer.append(f'<li value="{num}">' + re.sub(r'^\s*\d+\.\s+', '', line) + '</li>')
