    ...
    def run(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                # 直接调用子图 invoke（Studio 可展开查看内部 9 个子节点）
                # 替代 orchestrator.safe_run()，保留子图的完整可视化
                if orchestrator.graph:
                    result_state = orchestrator.graph.invoke(state_with_path)  # type: ignore[arg-type]
                else:
                    result_state = orchestrator.safe_run(state_with_path)
                
                # 仅在统一 retry 语义明确要求 full_cicd / rerun_multi_round 时才继续外层循环
                retry_result = result_state.get("cicd_retry_result", {})
                if retry_result.get("requested") and retry_result.get("retry_mode") in {"full_cicd", "rerun_multi_round"}:
                    retry_reason = retry_result.get("retry_reason", "未知原因")
                    print(f"  [CICDAgent] 检测到需要完全重试: {retry_reason}")
                    continue
        ...
        if result_state.get("errors"):
            return {
                **state,
                ...
                "organized_stage_details": result_state.get("organized_stage_details"),
                "report_md": result_state.get("report_md"),
                "architecture_json": result_state.get("architecture_json"),
