...
class ReviewerAgent(BaseAgent):
    ...
    def run(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            print("\n[3/6] 提取报告中的实体...")
            claimed = self._extract_claimed(report, ground_truth, ci_data, section_assignment_result)
            ...
            print("\n[5/7] 完整性检查...")
            completeness_issues = self._validate_completeness(report, ground_truth, claimed, section_assignment_result)
            ...
            if Path(report_html).exists():
                final_report_result = self.validate_final_reports(report_md, report_html, ci_data, section_assignment_result)
    ...
    def _count_job_rows_by_regex(self, report: str) -> int:
        """用正则统计报告中 Job 表格的数据行数（即实际列出的 Job 总数）"""
        pattern = r'^\| *\d+ *\|'
        return len(re.findall(pattern, report, re.MULTILINE))
    ...
    def _extract_claimed(self, report: str, ground_truth: GroundTruth, ci_data: Dict, section_assignment_result: Optional[Dict[str, Any]] = None) -> ClaimedEntities:
        ...
        assignment_sections = self._build_assignment_workflow_sections(section_assignment_result or {})
        if assignment_sections:
            claimed_workflows = set(assignment_sections.keys())
        ...
        claimed_triggers = {}
        workflow_data_map = self._build_assignment_workflow_data_map(section_assignment_result or {})
        for wf in claimed_workflows:
            wf_triggers = set()
            if wf in workflow_data_map:
                wf_triggers.update(workflow_data_map[wf].get("triggers", []))
            ...
            wf_section = assignment_sections.get(wf) or self._extract_workflow_section(report, wf)
            for trigger in valid_triggers:
        ...
        claimed_jobs = {}
        for wf in claimed_workflows:
            section = assignment_sections.get(wf) or self._extract_workflow_section(report, wf)
    ...
    def _build_assignment_workflow_sections(self, section_assignment_result: Dict[str, Any]) -> Dict[str, str]:
        sections: Dict[str, str] = {}
        for assignment in section_assignment_result.get("workflow_assignments", []) or []:
            workflow_id = assignment.get("workflow_id", "")
            detail_markdown = assignment.get("detail_markdown", "")
            if workflow_id and detail_markdown:
                sections[workflow_id] = detail_markdown
        return sections

    def _build_assignment_workflow_data_map(self, section_assignment_result: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        workflow_map: Dict[str, Dict[str, Any]] = {}
        for assignment in section_assignment_result.get("workflow_assignments", []) or []:
            workflow_id = assignment.get("workflow_id", "")
            if workflow_id:
                workflow_map[workflow_id] = assignment.get("workflow_data", {}) or {}
        return workflow_map
...
输出："""
    ...
    def _validate_completeness(
        ...
        ground_truth: GroundTruth,
        claimed: ClaimedEntities,
        section_assignment_result: Optional[Dict[str, Any]] = None,
    ) -> List[dict]:
        """完整性检查（使用正则统计 + 规则对比，不依赖 LLM）"""
        issues = []
        
        reported_wf_count = self._count_workflows_by_regex(report)
        if section_assignment_result and section_assignment_result.get("workflow_assignments"):
            reported_wf_count = len({a.get("workflow_id") for a in section_assignment_result.get("workflow_assignments", []) if a.get("workflow_id")})
        ...
        if actual_job_count > 0 and reported_job_count < actual_job_count:
            missing_count = actual_job_count - reported_job_count
            issues.append({
                "severity": "incomplete",
    ...
    def validate_final_reports(
        ...
        if Path(md_report_path).exists():
            ...
            # 1.2 检查工作流覆盖率
            expected_workflows = set(ci_data.get("workflows", {}).keys())
            found_workflows = set(re.findall(r'####\s+\d+\.\d+\s+([\w.-]+\.ya?ml)', md_content))
            if section_assignment_result and section_assignment_result.get("workflow_assignments"):
                found_workflows = {a.get("workflow_id") for a in section_assignment_result.get("workflow_assignments", []) if a.get("workflow_id")}
