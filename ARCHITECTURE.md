...
```
CICDOrchestrator 编排:
extract → plan → invoke → build_batch_input → merge → check → retry? → organize → report → summary

节点说明:
- extract:           DataExtractionAgent - 提取CI/CD配置数据
- plan:              AnalysisPlanningAgent - 决定多轮与分批分析策略，产出全局上下文
- invoke:            LLMInvocationAgent - 执行多轮调用与分批调用
- build_batch_input: LLMInvocationAgent（包装）- 构建多轮到分批之间的显式输入契约（batch_input_context）
- merge:             ResultMergingAgent - 执行结构化组装，产出最终结构化真相源（cicd_assembled_data）
- check:             QualityCheckAgent - 先执行阶段契约检查，再执行最终结构化产物校验
- retry:             RetryHandlingAgent - 统一生成重试决策对象 cicd_retry_result
- organize:          StageOrganizationAgent - 阶段组织，产出 section_assignment_result 和 report_artifacts
- report:            ReportGenerationAgent - 基于 report_artifacts 生成 Markdown 报告
- summary:           SummaryGenerationAgent - 基于 cicd_assembled_data 生成摘要 JSON
```

#### 3.3.1 CI/CD 结构化真相源

CI/CD 分析链路现在明确区分以下状态对象：

- `cicd_rounds_data`：多轮分析的逐轮结构化结果
- `batch_input_context`：多轮到分批之间的显式输入契约
- `cicd_batch_outputs_data`：分批调用结果
- `cicd_assembled_data`：最终唯一结构化真相源
- `cicd_retry_result`：统一重试决策对象
- `cicd_failure_data`：失败类型与失败范围说明

其中：

- `cicd_assembled_data` 是 **唯一最终结构化真相源**
- `merged_response` 降级为展示/调试/导出文本
- `merged_response` 不再允许作为 contract check、final validation、retry decision、route decision 的主输入

#### 3.3.2 多轮与分批的边界模型

多轮阶段可以影响分批阶段，但这种影响必须是**显式传递**的，只允许通过以下载体传递：

- `cicd_rounds_data`
- `batch_input_context`
- `batch_inputs`（显式 batch metadata，区分 workflow_detail / script_detail）
- LangGraph state 中显式声明的结构化字段

禁止以下行为：

- 通过 `merged_response` 反向抽取分批输入
- 通过未声明的 previous messages 共享分批上下文
- 通过隐式 LLM 会话记忆把多轮结果泄漏到后续 batch
...
CI/CD 链路中存在两层职责不同的检查：

1. **Contract Check（阶段契约检查）**
   - 位置：多轮完成后，分批开始前
   - 输入：`batch_input_context`
   - 目的：判断分批是否具备合法、稳定、显式的输入
   - 不负责最终业务验收

2. **Final Validation（最终产物校验）**
   - 位置：分批完成并 assembly 后
   - 输入：`cicd_assembled_data`
   - 目的：检查最终结构化产物是否完整、一致、可供下游消费

这两者必须分离，禁止把 contract check 写成 final validation 的提前版。

#### 3.3.4 统一重试状态模型

CI/CD 链路中的所有重试都必须通过 `cicd_retry_result` 驱动，至少包含：
...
推荐的 retry_mode 语义至少包括：

- `revalidate_only`：仅在当前已组装产物上复核
- `reassemble`：重新组装 / 再校验，不重新调用 LLM
- `rerun_batch`：仅重跑分批分析
- `rerun_multi_round`：重跑多轮主分析
- `full_cicd`：整段 CI/CD 重跑（仅保留给上游输入契约失效）

禁止继续使用以下碎片化模式作为主决策输入：

- 仅凭 `validation_result.needs_retry`
- 仅凭 `cicd_analysis.status=failed`
- 仅凭 `cicd_retry_mode`
- 仅凭 `cicd_retry_count`

#### 3.3.5 LangGraph 编排约束

CI/CD 子图中的顺序执行、多轮执行、分批执行、assembly、重试闭环都必须通过 LangGraph 表达。

`run_sequential` 已退役，当前实现仅返回错误信息，待后续清理时删除。新的业务逻辑不得再扩展或依赖它。

#### 3.3.6 报告契约与无损迁移

CI/CD 报告生成不再被视为“把 LLM 文本保存成 Markdown”这么简单，而是一个需要保持兼容语义的结构化装配过程。
...
#### 3.3.6.1 empty-script 合法空态

当项目未提取到任何脚本（`scripts=[]`）时，系统必须将其视为**合法空态**，而不是异常：

- `script_detail` 批次仍应有显式产物或显式空态说明
- `脚本目录索引` 章节必须保留
- `关键配置` 可以为空，但不能因此删除脚本相关章节
- validation 必须区分“合法空态”与“异常缺失”

也就是说，empty-script 只能表现为“空内容”，不能表现为“缺章节”或“隐式跳过脚本分析通道”

#### 3.3.6.2 batch_type 元数据

workflow-detail 与 script-detail 批次必须在生成阶段显式写入 batch metadata，例如：

- `batch_type=workflow_detail`
- `batch_type=script_detail`

消费端必须优先基于 metadata 路由；文件命名只允许作为调试和兼容 fallback，不得继续作为业务真相源。

这些章节不仅影响 Markdown，也影响：

- HTML 侧边导航与 section id
- Reviewer 对必需章节与 JSON marker 的校验
- 脚本目录索引与关键配置的展示完整性

因此，迁移目标不是简单移除正则，而是：

- 先冻结 `report_contract`
- 再生成 `report_artifacts`
- 最终让 Markdown / HTML / JSON 都从 `report_artifacts` 渲染

#### 3.3.7 报告结构化对象

新增以下与报告组织相关的结构化对象：

- `report_contract`：冻结当前报告的章节契约、顺序、owner、必需章节与 marker 约束
- `cicd_stage_mapping_data`：multi-round 产出的权威阶段映射
- `workflow_details_data`：batch 产出的 workflow 级明细
- `script_analysis_data`：batch 产出的脚本目录索引、关键配置、脚本分析内容
- `organized_stage_details`：按照权威阶段映射重组后的 workflow 明细
- `report_artifacts`：最终报告章节树与内容块，是 Markdown/HTML 的共同输入

- `section_assignment_result`：section/workflow 归属的单一真相源；上游一次性计算 workflow/script 到 section 的归属结果，下游 reporter/reviewer 优先消费它，不再各自从 Markdown 反向重算

#### 3.3.8 报告生成责任划分

- multi-round：负责全局骨架，如项目概述、阶段划分、整体架构与汇总类结论
- batch：负责 workflow/script/config 明细
- StageOrganization：负责先生成 `section_assignment_result`，再按权威阶段映射重组 workflow 明细
- Report artifacts assembly：负责拼装稳定章节树
- Markdown/HTML：只做渲染，不再推断事实

其中，Markdown 架构图章节必须直接渲染 multi-round 产出的 `architecture_diagram`；
`architecture_json` 继续作为 HTML / 结构化消费的真相源，但不得再用占位文案替代 Markdown 图内容。

#### 3.3.8.1 统一规则与 fallback

当前 section/workflow 匹配规则必须遵循：

- 主路径：消费 `section_assignment_result`
- fallback：仅在兼容旧报告/旧产物时，允许使用 regex 从 Markdown 反向提取

也就是说：

- `section_assignment_result` 是主判断依据
- regex 只允许作为兼容降级路径
- 不允许上游和下游继续各自维护一套独立主规则

#### 3.3.8.2 report-fix 契约

report-fix 采用 `plan/apply` 两阶段契约：

- analyze 入口必须显式提供 `interactive_mode`
- `ReportFixPlanAgent` 负责生成 `fix_plan` 与 `user_fix_choice`
- `ReportFixApplyAgent` 只消费 plan 结果，不应要求入口直接提供 `user_fix_choice`

非交互 analyze 路径中：

- `interactive_mode=False`
- plan 节点必须自动给出默认选择
- apply 节点只有在 plan 成功产出 choice 时才执行

#### 3.3.8.3 safe_run / Studio 边界说明

当前 CI/CD 主路径中仍存在“为了 Studio 子图展开而绕过 `CICDAgent` / `CICDOrchestrator` 父级 `safe_run()`”的架构妥协。

当前实现保留该妥协，以换取：

- CLI 可运行
- Studio 节点高亮可见
- `langgraph dev` 可视化入口可继续使用

这意味着父级 `safe_run` 边界不完整、Studio/runtime 图职责存在分叉，但当前阶段接受该瑕疵，不继续扩大该专项改造。

#### 3.3.9 无损迁移校验

最终校验除结构化完整性外，还必须覆盖：

- required section 是否存在
- section 内容是否为空
- workflow 是否都完成权威阶段重映射
- key config 数据是否落入 `脚本目录索引` / `关键配置`
- `ARCHITECTURE_JSON` / `ANALYSIS_SUMMARY` marker 是否仍按兼容要求存在
...
| **处理Agent** | HelpHandlerAgent | 处理 help 命令 |
| **处理Agent** | InsightsHandlerAgent | 处理 insights 命令 |
| **处理Agent** | RecommendHandlerAgent | 处理 recommend 命令 |
| **处理Agent** | SimilarHandlerAgent | 处理 similar 命令 |
| **处理Agent** | AnalyzersHandlerAgent | 处理 analyzers 命令 |
| **处理Agent** | VersionHandlerAgent | 处理 version 命令 |
| **处理Agent** | ClearHandlerAgent | 处理 clear 命令 |
| **处理Agent** | QuitHandlerAgent | 处理 quit 命令 |
...
| 路由函数 | 触发条件 | 下一步 |
|---------|---------|--------|
| route_after_extract | strategy=skip | END |
| route_after_extract | 其他 | plan |
| route_after_plan | strategy=skip | END |
| route_after_plan | 其他 | invoke |
| route_after_batch_input | context_status=ready | merge |
| route_after_batch_input | 其他 | retry |
| route_after_check | contract_check_result.status=failed | retry |
| route_after_check | validation_result.needs_retry=True | retry |
| route_after_check | retry_mode in (retry,supplement) 且有 retry_issues | retry |
| route_after_check | errors 且 retry_count < max_retries | retry |
| route_after_check | errors 且 retry_count >= max_retries | fail→END |
| route_after_check | 其他 | organize |
| route_after_retry | cicd_retry_result.requested=False | fail→END |
| route_after_retry | cicd_retry_result.exhausted=True | fail→END |
| route_after_retry | 其他 | invoke |
...
| 版本 | 日期 | 变更内容 |
|------|------|---------|
| 2.4.0 | 2026-04-19 | 修正文件结构（删除不存在的子图文件），补充缺失的 11 个 handler agents，修正 CI/CD 子图流程（补充 build_batch_input 节点），更新路由函数说明表 |
