...
```
CICDOrchestrator 编排:
extract → plan → invoke(多轮+分批) → build_batch_input → contract_check → merge/assembly → final_validation → retry? → organize → report → summary

子Agent职责:
- DataExtractionAgent: 提取CI/CD配置数据
- AnalysisPlanningAgent: 决定多轮与分批分析策略，并产出全局上下文
- LLMInvocationAgent: 执行多轮调用与分批调用，输出结构化轮次结果、批次结果和分批输入契约
- BatchInputBuildAgent: 构建多轮到分批之间的显式输入契约
- ResultMergingAgent: 执行结构化组装，产出最终结构化真相源
- QualityCheckAgent: 先执行阶段契约检查，再执行最终结构化产物校验
- RetryHandlingAgent: 统一生成重试决策对象 cicd_retry_result
- StageOrganizationAgent: 阶段组织
- ReportGenerationAgent: 基于结构化产物生成 Markdown 报告
- SummaryGenerationAgent: 基于结构化产物生成摘要 JSON
...
`run_sequential` 不再是正式业务路径，只允许作为待删除的历史兼容代码存在；新的业务逻辑不得再扩展或依赖它。
